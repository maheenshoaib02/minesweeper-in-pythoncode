...
import random
# code form Ashraf Ibrahim Youtube Channel
def click(index):
...
def game_over():                                       # <---- Function to show all tiles when the game is over
    global isgameover
    isgameover = True
    for i in range(row*col):
        if map[i] == "b":
...
def rightclick(event, index):                           # <---- Right click info
...
isgameover = False
row = 10                   # number of row
col = 10                   # number of column
bomb = 10                  # number of bomb
flags = bomb               # flag is equal to bomb
...
root.mainloop()                # <---- Main function for better organized code
